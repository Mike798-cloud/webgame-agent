...
export function buildExperienceExport(state){
  const categories=(state?.experienceCategories||[]).map((x,i)=>({id:x.id,name:x.name,description:x.description||'',order:i}));
  const exported=[],excluded=[];
  for(const e of state?.experiences||[]){
    const {record,leaks}=sanitizeExperienceForStorage(e,state?.projects||[],categories);
    if(leaks.length||record.needsReview){excluded.push({id:e.id,title:e.title||'',reason:leaks.length?'检测到项目标识':'标记为待整理'});continue;}
    ...
    if(record.createdAt)clean.createdAt=record.createdAt;if(record.updatedAt)clean.updatedAt=record.updatedAt;
    exported.push(clean);
  }
  return{payload:{schema:EXPERIENCE_SCHEMA,version:EXPERIENCE_SCHEMA_VERSION,exportedAt:new Date().toISOString(),categories,experiences:exported,stats:{exported:exported.length,excluded:excluded.length}},excluded};
}
