...
> v13.2：修复“新增经验被相似度判断自动合并，导致导出条数不增加”的问题；只有相似度达到明确合并阈值或用户确认时才合并。经验导出现在与经验库条数一一对应，不再静默排除待整理记录。
...
- **经验 JSON 与项目备份彻底分离**：`导出经验库` 只写入 `schema / version / categories / experiences / stats`，不包含项目、仓库扫描、部署地址、设置或密钥。
- **三种数据文件明确区分**：经验库、单项目、完整备份分别导入/导出；旧版完整备份仍可作为经验来源导入，但只抽取其中的经验。
- **可编辑分类体系**：经验分类使用稳定 `categoryId`；支持新建、改名、排序、合并/删除、逐条改类和批量移动。删除非空分类时必须先迁移经验，`未分类` 永久保留。
- **AI 重分类只读取经验**：重分类接口只接收经验与分类，不读取项目；优先复用已有分类，避免不断制造近义分类。
- **经验去项目化**：保存与导出前检查项目名、仓库/部署链接等专有信息；仍含项目标识或被标记待整理的条目不会进入纯经验导出。
- **旧数据兼容**：旧经验的中文 `category` 会迁移到稳定分类 ID；旧版完整 JSON 可合并到经验库而不会把项目一并导入。
- **项目范围经验不再全局泄漏**：历史 `scope=项目` 的经验保留供整理，但不会自动注入其他项目的 AI 上下文。

## v13.3 控件与任务修复

- 固定模型策略现在可同时选择供应商并填写具体模型 ID；前端会通过 `X-AI-Model` 发送给 Worker，留空时使用 Worker 中该供应商的默认模型。
- 经验库 AI 重分类、经验 AI 深审、作品集 AI 分类校正支持再次点击取消；取消只终止尚未完成的请求，不会把未确认结果写入经验库。
- 模型策略栏和经验库工具栏使用独立响应式布局，避免中等宽度窗口下拉框被挤成窄条、按钮溢出或被截断。
